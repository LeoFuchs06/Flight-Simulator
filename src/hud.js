...
export class HUD {
  ...
  _drawArtificialHorizon(ctx, cx, cy, pitch, roll) {
    ...
    ctx.lineWidth = 1.8;
    ctx.beginPath();
    ctx.moveTo(-140, 0); ctx.lineTo(-40, 0);
    ctx.moveTo(40, 0);   ctx.lineTo(140, 0);
    ctx.stroke();
    ...
    for (let deg = -60; deg <= 60; deg += 5) {
      ...
      ctx.beginPath();
      ctx.moveTo(-len, y); ctx.lineTo(-len + 20, y + (deg > 0 ? 6 : -6));
      ctx.moveTo( len, y); ctx.lineTo( len - 20, y + (deg > 0 ? 6 : -6));
      ctx.stroke();
      if (deg % 10 === 0) {
        ctx.setLineDash([]);
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(cx - 40, cy); ctx.lineTo(cx - 12, cy); ctx.lineTo(cx - 6, cy + 6);
    ctx.moveTo(cx +  6, cy + 6); ctx.lineTo(cx + 12, cy); ctx.lineTo(cx + 40, cy);
    ctx.stroke();
  }
  ...
  _drawBankIndicator(ctx, cx, cy, roll) {
    ...
    ctx.fillStyle = COLOR;
    ctx.beginPath();
    ctx.moveTo(0, -r + 2); ctx.lineTo(-5, -r + 12); ctx.lineTo(5, -r + 12);
  ...
  _drawHeadingTape(ctx, cx, y, heading) {
    ...
    for (let d = -60; d <= 60; d += 5) {
      ...
      ctx.strokeStyle = COLOR;
      ctx.beginPath();
      ctx.moveTo(x, -14); ctx.lineTo(x, major ? 0 : -6);
      ctx.stroke();
      if (major) {
